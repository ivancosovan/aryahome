    ...
    if (BX.Ctweb && BX.Ctweb.YandexDelivery) {
        ...
        BX.Ctweb.YandexDelivery.Controller = function(params) {
            ...
            this.InitMap = function () {
                ...
                if($(".yamaps-suggest-input").length>0){
                    ymaps.load(function () {
                        ...
                        var suggestView = new ymaps.SuggestView('suggest', {
                            ...
                            provider: {
                                suggest:(function(request, options){
                                    var suggestInput;
                                    suggestInput = ymaps.suggest( "Россия, " + request);
                                    // suggestInput.then(function (items) {
                                    //     for (let i = items.length; i--; ){
                                    //         var massElem = items[i].value;
                                    //         if (massElem.indexOf("Москва")==-1 && massElem.indexOf("Московская область")==-1) {
                                    //             items.splice(i, 1);
                                    //         }
                                    //     }
                                    //
                        ...
                        suggestView.events.add('select', function (e) {
                            searchControl.search(suggestView.state._data.request, {noPopup: true});
                        });
                    });
                }
            ...
            searchControl.events.add('resultselect', function (e) {
                ...
                var point = results[0].geometry.getCoordinates();
