      ...
      (BX.Sale.OrderAjaxComponent = {
              ...
              events: {
                onAfterPopupShow: BX.delegate(function () {
                  BX.cleanNode(this.loadingScreen.popupContainer),
                    ...
                    this.loadingScreen.popupContainer.appendChild(BX.create("IMG", { props: { src: this.templateFolder + "/images/loader.gif" } })),
        ...
        initOptions: function () {
          ...
          if (
            (this.initPropsListForLocation(),
              (this.propertyCollection = new BX.Sale.PropertyCollection(BX.merge({ publicMode: !0 }, this.result.ORDER_PROP))),
              (this.fadedPropertyCollection = new BX.Sale.PropertyCollection(BX.merge({ publicMode: !0 }, this.result.ORDER_PROP))),
        ...
        initPagination: function () {
          ...
          if (this.result.DELIVERY)
            if (((this.result.DELIVERY = this.getDeliverySortedArray(this.result.DELIVERY)), this.options.deliveriesPerPage > 0 && this.result.DELIVERY.length > this.options.deliveriesPerPage)) {
              for (arReserve = this.result.DELIVERY.slice(), pages = Math.ceil(arReserve.length / this.options.deliveriesPerPage), arPages = [], i = 0; i < pages; i++)
          ...
          if (this.result.PAY_SYSTEM)
            if (this.options.paySystemsPerPage > 0 && this.result.PAY_SYSTEM.length > this.options.paySystemsPerPage) {
              for (arReserve = this.result.PAY_SYSTEM.slice(), pages = Math.ceil(arReserve.length / this.options.paySystemsPerPage), arPages = [], i = 0; i < pages; i++)
        ...
        locationsCompletion: function () {
          ...
          for (i in ((this.locationsInitialized = !0), this.fixLocationsStyle(this.deliveryBlockId, this.regionHiddenBlockNode), this.fixLocationsStyle(this.propsBlockNode, this.propsHiddenBlockNode), this.locations)) {
            ...
            (section = BX.findParent(locationNode, { className: "bx-soa-section" })) && "true" == section.getAttribute("data-visited") && this.isValidProperty(data)),
                      ...
                      BX.create("INPUT", {
                        ...
                        events: {
                          click: BX.delegate(function (e) {
                            ...
                            return (
                              "Y" === arMaxOptions.THEME.LOGIN_EQUAL_EMAIL && login && email && (login.value = email.value), (BX("do_register").value = "Y"), this.sendRequest("showAuthForm"), BX.PreventDefault(e)
                            );
                          }, this),
                        },
                      }),
        ...
        createBasketItem: function (basketItemsNode, item, index, active) {
          var mainColumns = [],
            ...
            countCols = 0;
          for (
            (this.options.showPreviewPicInBasket || this.options.showDetailPicInBasket) && mainColumns.push(this.createBasketItemImg(item.data)), mainColumns.push(this.createBasketItemContent(item.data)), i = 0;
            ...
            4 == ++basketColumnIndex && this.result.GRID.HEADERS[i + 1] && (otherColumns.push(BX.create("DIV", { props: { className: "bx-soa-item-nth-4p1" } })), (basketColumnIndex = 0), dopColCount++),
              countCols++);
          if (active)
        ...
        createBasketItemContent: function (data) {
          var itemName = data.NAME || "",
            ...
            propsNodes = [];
          if (
            ("Y" !== this.params.HIDE_DETAIL_PAGE_URL && data.DETAIL_PAGE_URL && data.DETAIL_PAGE_URL.length && (titleHtml = '<a href="' + data.DETAIL_PAGE_URL + '">' + titleHtml + "</a>"),
        ...
        createBasketItemColumn: function (column, allData, active) {
          if (column && allData) {
            ...
            if ("PRICE_FORMATED" === column.id)
              ...
              this.options.showPriceNotesInBasket && active && (textNode.appendChild(BX.create("BR")), textNode.appendChild(BX.create("SMALL", { text: data.NOTES })));
            ...
            else if (BX.util.in_array(column.id, ["QUANTITY", "WEIGHT_FORMATED", "DISCOUNT_PRICE_PERCENT_FORMATED"])) textNode.appendChild(BX.create("SPAN", { html: data[column.id] }));
            else if ("PREVIEW_TEXT" === column.id)
              "html" === data.PREVIEW_TEXT_TYPE ? textNode.appendChild(BX.create("SPAN", { html: data.PREVIEW_TEXT || "" })) : textNode.appendChild(BX.create("SPAN", { text: data.PREVIEW_TEXT || "" }));
            else {
                ...
                for (var i in columnData)
                  ...
                  ("image" == columnData[i].type
                    ...
                    : "linked" == columnData[i].type
                      ...
                      : columnData[i].value && (textNode.appendChild(BX.create("SPAN", { html: columnData[i].value })), textNode.appendChild(BX.create("BR"))));
                val.length &&
                textNode.appendChild(
                  BX.create("DIV", {
                ...
                if (BX.type.isArray(columnData)) {
                  for (i in columnData)
                    if (columnData.hasOwnProperty(i))
                      ...
                      else if ("linked" == columnData[i].type) textNode.appendChild(BX.create("SPAN", { html: columnData[i].value_format })), textNode.appendChild(BX.create("BR"));
                      else {
        ...
        addCoupon: function (coupon) {
          for (
            var couponListNodes = this.orderBlockNode.querySelectorAll(".bx-soa-coupon:not(.bx-soa-coupon-item-fixed) .bx-soa-coupon-item"), i = 0;
            i < couponListNodes.length && !couponListNodes[i].querySelector('[data-coupon="' + BX.util.htmlspecialchars(coupon) + '"]');
            i++
          )
        ...
        getDeliveryLocationInput: function (node) {
          ...
          if ((location = this.locations[locationId]) && location[0] && location[0].output)
            for (k in ((this.regionBlockNotEmpty = !0),
              ...
              node.appendChild( // add title before loc input custom v1
                BX.create("DIV", {
                  props: {className: "bx-soa-location-subtitle"},
                  html: "<h6>1. Уточните ваш город</h6>",
                })
              ),
        ...
        getZipLocationInput: function (node) {
          ...
          if (zipProperty) {
            (this.regionBlockNotEmpty = !0),
              (propsItemNode = BX.create("DIV", { props: { className: "form-group bx-soa-location-input-container" } })).setAttribute("data-property-id-row", zipProperty.ID),
        ...
        editActivePaySystemBlock: function (activeNodeMode) {
          ...
          (this.opened["bx-soa-paysystem"] || this.paySystemBlockNode.classList.add("bx-step-completed-open"), // fix closed paysystem list, old class "bx-step-completed" custom v1
              ...
              BX.create("LABEL", {
                ...
                html:
                  ...
                  (this.uniqueText(item.DESCRIPTION)
                    ? "<div class='hint colored_theme_hover_bg-block'><span class='icon colored_theme_hover_bg-el'><i>?</i></span><div class='tooltip'>" + item.DESCRIPTION + "</div></div>"
                    : ""),
        ...
        editActiveDeliveryBlock: function (activeNodeMode) {
          ...
          (this.opened[this.deliveryBlockNode.id] || this.deliveryBlockNode.classList.add("bx-step-completed-open"), // fix closed delivery list, old class "bx-step-completed" custom v1
            ...
            this.showSlimPropInDelivery("FLOOR", deliveryNode),
            this.showSlimPropInDelivery("APARTAMENT", deliveryNode),
              this.showSlimPropInDelivery("ORDER_PVZ", deliveryNode),
              this.showSlimPropInDelivery("INFO_PVZ", deliveryNode),
        ...
        showPropInDelivery: function (propCode, node, title) { // add ADDRESS prop in Delivery block
          ...
          for (; (group = groupIterator()); )
            for (propsIterator = group.getIterator(); (property = propsIterator()); ) property.getSettings().CODE === propCode && (this.getPropertyRowNode(property, $propInner, !1, !1), (isAddProp = !0));
          isAddProp && node.appendChild($prop);
        },
        showSlimPropInDelivery: function (propCode, node, title) { // add user props in Delivery block
          if (!this.result.ORDER_PROP || !this.propertyCollection) return;
          var group,
            property,
            groupIterator = this.propertyCollection.getGroupIterator(),
          ...
          const $propInner = BX.create("DIV", {
            ...
            $prop = BX.create("DIV", {
              props: { className: "col-md-2 bx-soa-extraprops group-without-margin" },
              children: [$propInner],
            });
          let isAddProp = !1;
          for (; (group = groupIterator()); )
          ...
          if (this.result.DELIVERY && !(this.result.DELIVERY.length <= 0)) {
            ...
            deliveryItemsContainer.appendChild(
              BX.create("DIV", {
                ...
                html: "<h6>2. Как вам удобнее получить заказ?</h6>",
              })
            );
          ...
          extraServices.push(
            ...
            if (delivery.EXTRA_SERVICES.hasOwnProperty(i) && (currentService = delivery.EXTRA_SERVICES[i]).canUserEditValue) {
              if (-1 == currentService.editControl.indexOf("this.checked"))
                (serviceName = BX.create("LABEL", { html: BX.util.htmlspecialchars(currentService.name) + (currentService.price ? " (" + currentService.priceFormatted + ")" : "") })),
                0 == i && (brake = !0),
        ...
        editFadeDeliveryContent: function (node) {
          ...
          warningNode && warningNode.innerHTML && node.appendChild(warningNode.cloneNode(!0)),
          (selectedDelivery && selectedDelivery.NAME) || node.appendChild(BX.create("STRONG", { text: BX.message("SOA_DELIVERY_SELECT_ERROR") }));
        },
        ...
        editPickUpList: function (isNew) {
          if (this.pickUpPagination.currentPage && this.pickUpPagination.currentPage.length) {
            ...
            if (
              ...
              found = !0;
            else if ((selectedDelivery = this.getSelectedDelivery()) && selectedDelivery.STORE) for (i = 0; i < selectedDelivery.STORE.length; i++) selectedDelivery.STORE[i] == selectedStore && (found = !0);
          ...
          if (!propsErrors.length)
            if (
              ...
              BX.ajax({
                ...
                onsuccess: BX.delegate(function (result) {
                  this.endLoader(),
                  result.profileID && ((this.opened[this.propsBlockNode.id] = !1), (this.opened.edit_profile = !1), this.showTempProfileInput(result), this.sendRequest("", { "change-profile": "Y" })),
        ...
        showPropItem: function (profile) {
          const innerInfo = [],
            propForName = this.getPropByFilter({ USER_PROPS: "Y", IS_PROFILE_NAME: "Y" });
          ...
          let isBottomProps = !1;
          const bottomProps = [],
            propForEmail = this.getPropByFilter({ USER_PROPS: "Y", IS_EMAIL: "Y" });
          let emailValue = this.getInputValueForProp(propForEmail);
          if (
            (profile && this.result.USER_PROFILES_PROPS && this.result.USER_PROFILES_PROPS[profile.ID] && this.result.USER_PROFILES_PROPS[profile.ID].EMAIL && (emailValue = this.result.USER_PROFILES_PROPS[profile.ID].EMAIL),
              emailValue)
          ) {
          ...
          let phoneValue = this.getInputValueForProp(propForPhone),
            changeProfile;
          if (
            (profile && this.result.USER_PROFILES_PROPS && this.result.USER_PROFILES_PROPS[profile.ID] && this.result.USER_PROFILES_PROPS[profile.ID].PHONE && (phoneValue = this.result.USER_PROFILES_PROPS[profile.ID].PHONE),
              phoneValue)
          ) {
          ...
          if (this.result.ORDER_PROP && this.propertyCollection) {
            for (
              ...
              for (propsIterator = group.getIterator(); (property = propsIterator()); )
                ...
                "STREET" !== property.getSettings().CODE && // skip custom delivery properties custom v1
                "HOUSE" !== property.getSettings().CODE &&
                "CORPUS" !== property.getSettings().CODE &&
                "INTERCOM" !== property.getSettings().CODE &&
                "PODEZD" !== property.getSettings().CODE &&
                "FLOOR" !== property.getSettings().CODE &&
        ...
        getPropertyRowNode: function (property, propsItemsContainer, disabled, with_column) {
          // console.log(property.getName()); user property custom v1
        ...
        insertLocationProperty: function (property, propsItemNode, disabled) {
          ...
          if (property.getId() in this.locations)
            if (disabled) {
              ...
              for (propContainer = BX.create("DIV", { props: { className: "soa-property-container" } }), propRow = this.locations[property.getId()], i = 0; i < propRow.length; i++)
          ...
          if (this.totalInfoBlockNode && this.result.TOTAL) {
            ...
            if (
              (BX.cleanNode(this.totalInfoBlockNode),
                ...
                this.options.showPayedFromInnerBudget
                  ? (this.totalInfoBlockNode.appendChild(this.createTotalUnit(BX.message("SOA_SUM_IT"), total.ORDER_TOTAL_LEFT_TO_PAY_FORMATED, { total: !0 })),
                  ...
                  : this.totalInfoBlockNode.appendChild(this.createTotalUnit(BX.message("SOA_SUM_IT"), total.ORDER_TOTAL_PRICE_FORMATED, { total: !0 })),
            ...
            if (
              ...
              total.DISCOUNT_PERCENT_FORMATED && parseFloat(total.DISCOUNT_PERCENT_FORMATED) > 0 && (discText += total.DISCOUNT_PERCENT_FORMATED),
                this.totalInfoBlockNode.appendChild(this.createTotalUnit(discText + ":", total.DISCOUNT_PRICE_FORMATED, { highlighted: !0 }))),
              ...
              conditions.length > 0 &&
              (conditions.unshift(BX.create("span", { text: BX.message("CONDITIONS_TEXT") })),
          ...
          if (curDelivery && (this.totalInfoBlockNode.appendChild(this.createTotalScrollInfo(BX.message("ORDER_TOTAL_DELIVERY_TITLE"), curDelivery.NAME, this.deliveryBlockNode)), curDelivery.PERIOD_TEXT)) {
            ...
            this.totalInfoBlockNode.appendChild(
              BX.create("div", {
                ...
                html: curDeliveryTotal, // change text type on html type for correct insert info custom v1
              })
            );
          }
        },
